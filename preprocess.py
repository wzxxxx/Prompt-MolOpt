...
def get_preprocess_parser(): #获取参数
...
def tokenize(fns: Dict[str, List[Tuple[str, str]]], output_path: str, repr_start: str, repr_end: str):
    ...
    # }
    #  args.train_src =  './data/SINGLE_OPT/src-train.txt'
    ...
    # 输入是smiles， 输出的表征形式是smiles或者selfies

    ofns = {}

    for phase, file_list in fns.items():
        #phase = 'train'
        # file_list = [(args.train_src, args.train_tgt)]
        ofns[phase] = []
...
def preprocess_main(args):
    ...
    # 检测是否做了tokenize，如果没有做tokenize，则做tokenize
    if args.do_tokenize:
