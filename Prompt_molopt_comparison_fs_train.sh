#!/bin/bash
#SBATCH -J fs_comparison_mol_opt
#SBATCH -o fs_comparison_mol_opt-%j.log
#SBATCH -e fs_comparison_mol_opt-%j.err
#SBATCH -N 1
#SBATCH -p gpu
...
python train_linux_prompt.py \
  ...
  --task="$TASK" \
  --load_from="$LOAD_FROM" \
  --train_bin="./preprocessed/fs_comparison_mol_opt_data/train_0.npz" \
  --valid_bin="./preprocessed/fs_comparison_mol_opt_data/val_0.npz" \
  --log_file="fs_comparison_mol_opt_train.log" \
  --vocab_file="./preprocessed/fs_comparison_mol_opt_data/fs_comparison_vocab_smiles.txt" \
  --save_dir="./checkpoints/fs_comparison_mol_opt_data" \
  --embed_size=256 \
