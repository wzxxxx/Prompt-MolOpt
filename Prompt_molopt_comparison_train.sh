#!/bin/bash
#SBATCH -J comparison_mol_opt
#SBATCH -o comparison_mol_opt-%j.log
#SBATCH -e comparison_mol_opt-%j.err
#SBATCH -N 1
#SBATCH -p gpu
...
python train_linux_prompt.py \
  ...
  --task="$TASK" \
  --load_from="$LOAD_FROM" \
  --train_bin="./preprocessed/comparison_mol_opt_data/train_0.npz" \
  --valid_bin="./preprocessed/comparison_mol_opt_data/val_0.npz" \
  --log_file="comparison_mol_opt_train.log" \
  --vocab_file="./preprocessed/comparison_mol_opt_data/comparison_vocab_smiles.txt" \
  --save_dir="./checkpoints/comparison_mol_opt_data" \
  --embed_size=256 \
