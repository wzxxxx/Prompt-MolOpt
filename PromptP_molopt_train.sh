#!/bin/bash
#SBATCH -J Remark_fs_MGA_v2
#SBATCH -o Remark_fs_MGA_v2-%j.log
#SBATCH -e Remark_fs_MGA_v2-%j.err
#SBATCH -N 1
#SBATCH -p gpu
#SBATCH --cpus-per-task=2
#SBATCH --gres=gpu:1
#SBATCH -w gpu16  # If you want to specify a computing node, you can write its name here and remove the first #
...
/home/zhenxing/env/s2s/bin/python train_linux_prompt.py \
  --model="$MODEL" \
  --data_name="$DATASET" \
  --task="$TASK" \
  --load_from="$LOAD_FROM" \
  --train_bin="./preprocessed/fs_mga_remark_data/train_0.npz" \
  --valid_bin="./preprocessed/fs_mga_remark_data/val_0.npz" \
  --log_file="fs_mga_remark_train.log" \
  --vocab_file="./preprocessed/fs_mga_remark_data/remark_vocab_smiles.txt" \
  --save_dir="./checkpoints/fs_mga_remark_data" \
  --embed_size=256 \
