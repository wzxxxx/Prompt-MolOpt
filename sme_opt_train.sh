#!/bin/bash
#SBATCH -J SME_opt
#SBATCH -o SME_opt-%j.log
#SBATCH -e SME_opt-%j.err
#SBATCH -N 1
#SBATCH -p gpu
...
/home/zhenxing/env/s2s/bin/python train_linux_prompt.py \
  ...
  --task="$TASK" \
  --load_from="$LOAD_FROM" \
  --train_bin="./preprocessed/s2s_sme_data/train_0.npz" \
  --valid_bin="./preprocessed/s2s_sme_data/val_0.npz" \
  --log_file="s2s_sme_train.log" \
  --vocab_file="./preprocessed/s2s_sme_data/vocab_smiles.txt" \
  --save_dir="./checkpoints/s2s_sme_data" \
  --embed_size=256 \
