    ...
    for task_name in task_names:
        ...
        for i in range(1, 6):
            ...
            if add_tag_task in ['BBBP']:
                ...
                if add_tag_task == '':
                    len_data_d = 10 ** 10
                data_i_succeed = data_i[data_i[f'opt smi {i} pred {origin_task}'] < 0.5]
                data_d_i_succeed = data_d_i[data_d_i[f'opt smi {i} pred {origin_task}'] < 0.5]
                data_i = data_i[data_i[f'src pred {add_tag_task}']<0.5]
                data_d_i = data_d_i[data_d_i[f'src pred {add_tag_task}']<0.5]
                # print(len(data), len(data_d))
                add_data_i_succeed = data_i[data_i[f'opt smi {i} pred {add_tag_task}'] >= 0.5]
                add_data_d_i_succeed = data_d_i[data_d_i[f'opt smi {i} pred {add_tag_task}'] >= 0.5]
                add_data_i_succeed = data_i_succeed[data_i_succeed[f'opt smi {i} pred {add_tag_task}']>=0.5]
            ...
            if add_tag_task in ['ESOL', 'lipop']:
                ...
                if origin_task in ['Mutagenicity', 'hERG']:
                    ...
                    data_i_succeed = data_i[data_i[f'opt smi {i} pred {origin_task}'] >= 0.5]
                    data_d_i_succeed = data_d_i[data_d_i[f'opt smi {i} pred {origin_task}'] >= 0.5]
                add_data_i_succeed = data_i[data_i[f'src pred {add_tag_task}']-data_i[f'opt smi {i} pred {add_tag_task}']<=-0.5]
                add_data_d_i_succeed = data_d_i[data_d_i[f'src pred {add_tag_task}']-data_d_i[f'opt smi {i} pred {add_tag_task}']<=-0.5]
                add_data_i_succeed = data_i_succeed[data_i_succeed[f'src pred {add_tag_task}']-data_i_succeed[f'opt smi {i} pred {add_tag_task}']<=-0.5]
        ...
        print(f'{task_name} {origin_task} {add_tag_task} {round(data_t_succeed/data_t_all_num, 3)}')
