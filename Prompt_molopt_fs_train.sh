#!/bin/bash
#SBATCH -J MGA_opt
#SBATCH -o MGA_opt-%j.log
#SBATCH -e MGA_opt-%j.err
#SBATCH -N 1
#SBATCH -p gpu
...
python train_linux_prompt.py \
  ...
  --task="$TASK" \
  --load_from="$LOAD_FROM" \
  --train_bin="./preprocessed/fs_mga_data/train_0.npz" \
  --valid_bin="./preprocessed/fs_mga_data/val_0.npz" \
  --log_file="fs_mga_train.log" \
  --vocab_file="./preprocessed/s2s_mga_data/vocab_smiles.txt" \
  --save_dir="./checkpoints/fs_mga_data" \
  --embed_size=256 \
