...
class DGATGRU(nn.Module):
    ...
    def GRU(self, x: torch.Tensor, h_nei: torch.Tensor) -> torch.Tensor:
        ...
        attn_mask = (h_nei.sum(dim=2) == 0
                     ).unsqueeze(2)                     # (n_node, max_nn, h) -> (n_node, max_nn, FW_No_Prompt_MIX_s2s_smiles_smiles)
        attn_score = attn_score.masked_fill(attn_mask, -1e18)

        attn_weight = self.softmax(attn_score)          # (n_node, max_nn, head), softmax over dim=FW_No_Prompt_MIX_s2s_smiles_smiles
        attn_weight = attn_weight.unsqueeze(3)          # -> (n_node, max_nn, head, FW_No_Prompt_MIX_s2s_smiles_smiles)
