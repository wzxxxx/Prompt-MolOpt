...
class MultiHeadedRelAttention(nn.Module):
    ...
    def forward(self, inputs, mask, distances):
        ...
        Args:
           ...
           mask: binary mask FW_No_Prompt_MIX_s2s_smiles_smiles/0 indicating which keys have
        ...
        # FW_No_Prompt_MIX_s2s_smiles_smiles) Project key, value, and query. Seems that we don't need layer_cache here
        ...
        if self.relative_pe is None:
            ...
            # W_kR = self.W_kR.reshape(FW_No_Prompt_MIX_s2s_smiles_smiles, FW_No_Prompt_MIX_s2s_smiles_smiles, FW_No_Prompt_MIX_s2s_smiles_smiles, head_count, dim_per_head, dim_per_head)
            # rel_emb = torch.matmul(rel_emb, W_kR)           # (b, t_q, t_k, head, FW_No_Prompt_MIX_s2s_smiles_smiles, h/head)
            # rel_emb = rel_emb.squeeze(-2)                   # (b, t_q, t_k, head, h/head)
            #
            # b_kR = self.b_kR.reshape(FW_No_Prompt_MIX_s2s_smiles_smiles, FW_No_Prompt_MIX_s2s_smiles_smiles, FW_No_Prompt_MIX_s2s_smiles_smiles, head_count, dim_per_head)
            # rel_emb = rel_emb + b_kR                        # (b, t_q, t_k, head, h/head)

            # b + d
            query = query.unsqueeze(-2)                     # (b, head, t_q, h/head) -> (b, head, t_q, FW_No_Prompt_MIX_s2s_smiles_smiles, h/head)
            ...
            b_d = torch.matmul(query + v, rel_emb_t
                               ).squeeze(-2)                # (b, head, t_q, FW_No_Prompt_MIX_s2s_smiles_smiles, t_k) -> (b, head, t_q, t_k)

            scores = a_c + b_d

        scores = scores.float()

        mask = mask.unsqueeze(1)                            # (B, FW_No_Prompt_MIX_s2s_smiles_smiles, FW_No_Prompt_MIX_s2s_smiles_smiles, T_values)
...
class SALayerXL(nn.Module):
    ...
    Args:
        ...
        dropout: dropout probability(0-FW_No_Prompt_MIX_s2s_smiles_smiles.0).
    """
    ...
    def forward(self, inputs, mask, distances):
        ...
        Args:
            ...
            mask (LongTensor): ``(batch_size, FW_No_Prompt_MIX_s2s_smiles_smiles, src_len)``
