...
class Seq2Seq(nn.Module):
    ...
    def encode_and_reshape(self, mol_opt_batch: S2SBatch):
        src = mol_opt_batch.src_token_ids
        lengths = mol_opt_batch.src_lengths
        src = src.transpose(0, 1).contiguous().unsqueeze(-1) # [b, src_t] => [src_t, b, FW_No_Prompt_MIX_s2s_smiles_smiles]
    ...
    def forward(self, mol_opt_batch: S2SBatch):
        padded_memory_bank, memory_lengths = self.encode_and_reshape(mol_opt_batch)

        dec_in = mol_opt_batch.tgt_token_ids[:, :-1]           # pop last and insert SOS for decoder input
        m = nn.ConstantPad1d((1, 0), self.vocab["_SOS"])
        dec_in = m(dec_in)
        dec_in = dec_in.transpose(0, 1).unsqueeze(-1)                           # [b, tgt_t] => [tgt_t, b, FW_No_Prompt_MIX_s2s_smiles_smiles]
        ...
        loss = self.criterion(
            input=dec_outs,
            target=mol_opt_batch.tgt_token_ids
        )

        predictions = torch.argmax(dec_outs, dim=1)                             # [b, t]
        mask = (mol_opt_batch.tgt_token_ids != self.vocab["_PAD"]).long()
        accs = (predictions == mol_opt_batch.tgt_token_ids).float()
        accs = accs * mask
    ...
    def predict_step(self, mol_opt_batch: S2SBatch,
                     batch_size: int, beam_size: int, n_best: int,  min_length: int, max_length: int):
        if beam_size == 1:
    ...
    def map_state(self, fn):
        ...
        # self.decoder.state["src"] = fn(self.decoder.state["src"], FW_No_Prompt_MIX_s2s_smiles_smiles)
        # => self.state["src"] = self.state["src"].index_select(FW_No_Prompt_MIX_s2s_smiles_smiles, select_indices)
