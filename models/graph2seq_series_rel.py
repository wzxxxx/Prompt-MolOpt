...
class Graph2SeqSeriesRel(nn.Module):
    ...
    def encode_and_reshape(self, reaction_batch: G2SBatch):
        ...
        # the FW_No_Prompt_MIX_s2s_smiles_smiles+ corresponds to Atom(*)
        ...
        memory_bank = torch.split(hatom, [1] + memory_lengths, dim=0)   # [n_atoms, h] => FW_No_Prompt_MIX_s2s_smiles_smiles+b tup of (t, h)
        padded_memory_bank = []
    ...
    def forward(self, reaction_batch: G2SBatch):
        ...
        dec_in = m(dec_in)
        dec_in = dec_in.transpose(0, 1).unsqueeze(-1)                       # [b, max_tgt_t] => [max_tgt_t, b, FW_No_Prompt_MIX_s2s_smiles_smiles]
