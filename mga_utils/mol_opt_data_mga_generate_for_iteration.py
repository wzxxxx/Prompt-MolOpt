...
from mga_utils.maskgnn import EarlyStopping, run_a_train_epoch, \
    run_an_eval_epoch, set_random_seed, collate_molgraphs_pred, Meter, MGA
import torch as th
from mga_utils.build_data import MolGraphDataset
from torch.utils.data import DataLoader
from mga_utils.maskgnn import set_random_seed
import warnings
...
def build_s2s_data(smiles_list, iteration, task_name, model_name, tag_list, vocab, s2s_file='', batch_size=128):
    src_smis = smiles_list
    tgt_smis = smiles_list
    tag_name_list = [task_name for smi in src_smis]
    src_atom_n = [Chem.MolFromSmiles(smi).GetNumAtoms() for smi in src_smis]
    max_len = 150
    src_token_ids = []
    src_lens = []
    tgt_token_ids = []
    ...
    np.savez(f'{s2s_file}{task_name}_{iteration}.npz',
...
if __name__ == '__main__':
    ...
    # parameter
    model_name = 'QED_DRD2_data_for_MGA'
    ...
    build_s2s_data(smiles_list, iteration, task_name, model_name, tag_list, vocab, s2s_file='', batch_size=128)
