...
def return_mol_opt_smi(smi, type='src'):
    ...
    for broken_bond_indice in broken_bond_indices:
        ...
        for atom_idx in broken_bond_indice:
            ...
            # 为虚拟节点添加表示是否改动
            if mol.GetAtomWithIdx(atom_idx).GetAtomMapNum() == 0:
...
def preprocess_opt_data(src_smi, tgt_smi):
    try:
        remark_src_smi = return_remark_src_smi(src_smi, tgt_smi)
        src_smi = return_mol_opt_smi(remark_src_smi, type='src')
        src_atom_index = return_mol_opt_smi_atom_index(remark_src_smi)
        tgt_smi = return_mol_opt_smi(tgt_smi, type='tgt')
        return src_smi, tgt_smi, src_atom_index
    except:
        print(f'SRC: {src_smi} TGT:{tgt_smi} is preprocessed failed.')
        return 'CC', 'CC', []
...
def build_fs_data(data_file, model_name, tag_list, vocab, group='train', max_len=150, fs_file='a.csv', batch_size=128, test_file='a_test.csv'):
    data = pd.read_csv(data_file)
    ...
    cano_tgt_smis = data.cano_tgt_smi.tolist()
    ...
    for i in range(len(src_smis)):
        ...
        src_smi, tgt_smi, src_atom_index = preprocess_opt_data(src_smis[i], tgt_smis[i])
        retain_sub = src_smi.split("[Opt Frag]")[0]
        opt_frag = tgt_smi
        retain_subs.append(retain_sub)
        opt_frags.append(opt_frag)
        if src_smi=='CC':
            atom_idx_start = atom_idx_end
        else:
            src_token = tag_name_list[i].split() + ['[Pharmacophores]'] + tokenize_smiles(src_smi)
            src_token_ids_i, src_lens_i, token_mga_feats_i = get_src_token_ids(src_token, vocab, src_embed_i, tag_indicator, max_len, src_atom_index)
    ...
    if group=='test':
        ...
        data['Opt Frag'] = opt_frags
        data.to_csv(test_file, index=False)
    del data, src_smis, tgt_smis, cano_src_smis, cano_tgt_smis, src_embed, retain_subs, opt_frags
...
if __name__ == '__main__':
    ...
    data_file = '../data/origin_data/Prompt_MolOpt_train_val_ADMET_few_shot.csv'
    test_data_file = '../data/origin_data/test_for_ADMET.csv' #data file with test
    vocab_file = '../preprocessed/fs_mga_remark_data/remark_vocab_smiles.txt'
    train_fs_file = '../preprocessed/fs_mga_remark_data/train_0.npz'
    val_fs_file = '../preprocessed/fs_mga_remark_data/val_0.npz'
    test_fs_file = '../preprocessed/fs_mga_remark_data/test_0.npz'
    test_file = '../preprocessed/fs_mga_remark_data/test_0.csv'
    max_len=150
    batch_size=128
    ...
    remove_mismatch(test_data_file, vocab)
    print('Remove mismatch is over.!')
